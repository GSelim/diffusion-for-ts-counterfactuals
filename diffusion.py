...
def Upsample(dim):
    return nn.ConvTranspose2d(dim, dim, (config.ConvKernelSize,1), config.ConvStrideSize, (config.ConvPaddingSize,0))

def Downsample(dim):
    return nn.Conv2d(dim, dim, (config.ConvKernelSize,1), config.ConvStrideSize, (config.ConvPaddingSize,0))
...
class Block(nn.Module):
    def __init__(self, dim, dim_out, groups = 8):
        ...
        self.proj = nn.Conv2d(dim, dim_out, (config.blockConvKernelSize,1), padding = (config.blockConvPadding,0))
...
class ConvNextBlock(nn.Module):
    ...
    #for images but is it adapted for time series ? 
...
schedules={ "cosine":cosine_beta_schedule,
            "linear":linear_beta_schedule,
            "quadratic":quadratic_beta_schedule,
            "sigmoid":sigmoid_beta_schedule}

# define beta schedule
betas = schedules[config.schedule](timesteps=config.timesteps)
...
# forward diffusion from 0 to t (bruitage de la  donnée x_start)
